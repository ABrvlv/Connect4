...
class DQNLightning(pl.LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], nb_batch) -> OrderedDict:
        ...
        self.log("wins", self.wins)
        self.log("losses", self.losses)
        self.log("draws", self.draws)
        self.log("total_reward", self.total_reward)
        self.log("games", self.games)
        self.log("loss", loss)
    ...
    def add_model_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument("--env", type=str, default="Connect4-v1", help="gym environment tag")
...
def main(args) -> None:
    ...
    trainer = pl.Trainer(accelerator="gpu", devices=1, max_epochs = -1, enable_checkpointing = True)

    trainer.fit(model)
    # trainer.fit(model, ckpt_path = "lightning_logs/version_26/checkpoints/epoch=269907-step=1889356.ckpt")
